...
describe "The PHP Platform Installer" do
	...
	describe "during a build" do
		context "of a project that has invalid platform dependencies" do
			let(:app) {
				new_app_with_stack_and_platrepo('test/fixtures/mycomposer.json',
					config: { "COMPOSER" => "mycomposer.json" },
					before_deploy: -> { system("composer require --quiet --ignore-platform-reqs 'ext-foo:*' && composer require --quiet --ignore-platform-reqs --no-update 'ext-bar:*'") or raise "Failed to require dummy extensions" },
					run_multi: true,
					allow_failure: true
				)
			}
