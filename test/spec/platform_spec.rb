	...
	describe "Composer Plugin" do
		before(:all) do
			...
			Dir.chdir(@install_tmpdir) do
				cmd = "exec {PHP_PLATFORM_INSTALLER_HUMAN_LOG_FDNO}> #{@humanlog_tmpfile.path}; export PHP_PLATFORM_INSTALLER_HUMAN_LOG_FDNO; export_file_path=#{@export_tmpfile.path} profile_dir_path=#{@profiled_tmpdir} composer install --no-dev"
				@stdout, @stderr, @status = Open3.capture3("bash -c #{Shellwords.escape(cmd)}")
			end
		end
