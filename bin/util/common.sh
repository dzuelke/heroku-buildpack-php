...
error() {
	local prefix=$(printf "%-${INDENT:-7}s" " !") # " !     " prefix
	...
	indent no_first_line_indent "$prefix"
	if [[ -s "$_captured_warnings_file" ]]; then
		echo "" | indent "" "$prefix"
		echo -e "\033[1;33mREMINDER:\033[1;31m the following \033[1;33mwarnings\033[1;31m were emitted during the build;" | indent "" "$prefix"
		echo "check the details above, as they may be related to this error:" | indent "" "$prefix"
		cat "$_captured_warnings_file" | indent "" "$(echo -e "${prefix}\033[1;33m-\033[1;31m ")"
	fi
...
warning() {
	...
	indent no_first_line_indent "${prefix}" | tee >(head -n1 >> "$_captured_warnings_file"; cat > /dev/null)
	echo -e "\033[0m" # reset style
}
